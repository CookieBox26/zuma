...
> [!CAUTION]
> 2026/02/24 に、利便性向上のため後方互換性のない修正をしました。従来形式の台本ファイルから動画を生成するには dac374b 以前のコミットを取得してください。
...
| field | description |
| ---- | ---- |
| back_img | 背景画像 png のパスを指定します。この画像のサイズが動画のサイズになります。場面ごとに背景画像を変えても構いませんが、全場面で同じサイズの画像にしてください。無地の背景でよい場合は back_size, back_color による指定もできます **(back_img と back_size + back_color のいずれかは必須)**。 |
...
| front_imgs | 任意で追加の画像を重ねられます (予め重ねた画像を back_img に指定しても同じですがこちらを使用する方が便利な場合に使用してください)。`{ path = 画像パス, coord = 表示座標, scale = スケール }` の配列を指定してください。 |
...
> [!CAUTION]
> こちらの台本は dac374b 以前向けのままになっており、まだ 2026/02/24 以降版の台本形式に修正できていません。
